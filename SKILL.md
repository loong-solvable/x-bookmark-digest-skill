...
它现在内置了一份只做 bookmark 提取的最小化 `opencli` 风格导出器，并且把 SQLite 读取也 vendoring 在 skill 里了，不依赖外部 `opencli` / `bb-browser` skill，也不依赖系统 `sqlite3`。运行时会：
...
这个 skill 只有在这两个 `report/*.md` 文件都写出来之后，才算任务完成。不要在只导出原始数据时就停下。
